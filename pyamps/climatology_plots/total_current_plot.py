...
modelv = AMPS(v, B, B, 0, F107, minlat = 60) # used to calcualte vector fields (feather plot)
models = AMPS(v, B, B, 0, F107, minlat = 60) # used to calculate scalar fields (contour plots)
...
for tilt in tilts:#tilt = tilts[0]
    ...
    for i in range(3): 
        for j in range(3):
            ...
            axes[i][j].featherplot(mlatv, mltv, jnv, jev, SCALE = 100, markersize = 1, markercolor = 'black', color = 'black', linewidth = .5, unit = None)
    ...
    plt.savefig('amps_total_current_' + str(tilt) + '_deg_tilt.png', dpi = 250)
    plt.savefig('amps_total_current_' + str(tilt) + '_deg_tilt.pdf')
