...
# used to calcualte vector fields (feather plot)
modelv = AMPS(v, B, B, 0, F107, minlat=60)
# used to calculate scalar fields (contour plots)
...
ca = {(0, 1): 0, (0, 0): -45, (1, 0): -90, (2, 0): -135,
      (2, 1): 180, (0, 2): 45, (1, 2): 90, (2, 2): 135}
...
for tilt in tilts:  # tilt = tilts[0]
    ...
    for i in range(3):
        for j in range(3):
            ...
            axdial.text(np.sin(ca[i, j] * d2r), np.cos(ca[i, j] * d2r), str(
                ca[i, j]) + r'$^\circ$', rotation=-ca[i, j], ha='center', va='center')
            axdial.plot([.75*np.sin(ca[i, j] * d2r), .85*np.sin(ca[i, j] * d2r)],
                        [.75*np.cos(ca[i, j] * d2r), .85*np.cos(ca[i, j] * d2r)], color='lightgrey')
            ...
            axes[i][j].contourf(mlats, mlts, jtot, levels=clevels,
                                extend='both', cmap=plt.cm.viridis)
            axes[i][j].featherplot(mlatv, mltv, jnv, jev, SCALE=100, markersize=1,
                                   markercolor='black', color='black', linewidth=.5, unit=None)
            axes[i][j].write(60, 10, '*' + str(int(np.round(jtot.max()))
                                               ) + ' mA/m', ha='left', va='bottom', size=9)
            axes[i][j].scatter(mlats[jtot.argmax()], mlts[jtot.argmax()], marker=(
                6, 2, 0), s=30, c='black', zorder=100)
            axes[i][j].scatter(mlats[jtot.argmax()], mlts[jtot.argmax()], marker=(
                6, 2, 0), s=30, c='gold', linewidth=.5, zorder=101)

    axdial.text(0, 0, u'TOTAL\nCURRENT', ha='center', va='center', size=14)

    plt.subplots_adjust(hspace=.01, wspace=.01, left=.01,
                        right=.99, bottom=.05, top=.99)
    ...
    axtemplate.plot([.8, 1], [0, 0], color='black',
                    linestyle='-', linewidth=.9)
    axtemplate.scatter(1, 0, marker='o', s=4, c='black')
    axtemplate.text(.75, 0, '200 mA/m', ha='right', va='center', size=9)

    axcbar.contourf(clevels, [0, 1], np.vstack(
        (clevels, clevels)), levels=clevels, cmap=plt.cm.viridis)
    axcbar.set_yticks([])
    axcbar.set_xlabel('mA/m')

    axinfo.text(axinfo.get_xlim()[0], axinfo.get_ylim()[
                0], '$|B|$ = %s nT, $v$ = %s km/s,\n F$_{10.7}$ = %s, TILT $= %s^\circ$' % (B, v, F107, tilt), ha='left', va='top', size=9)
    plt.savefig('amps_total_current_latest_' +
                str(tilt) + '_deg_tilt.png', dpi=250)
    plt.savefig('amps_total_current_latest_' + str(tilt) + '_deg_tilt.pdf')
