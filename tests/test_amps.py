...
class Test_AMPS(object):
    ...
    def test_calculate_matrices(self,amps_model):
        ...
        assert_allclose(model.tor_sinmphi_vector[2],[0,np.sqrt(2)/2,0,np.sqrt(2)/2,1])
        assert_allclose(model.pol_cosmphi_vector[0],[1.000000, 0.987688, 1.000000, 0.987688, 0.951056],atol=1e-6)

        assert_allclose(model.pol_P_scalar[1],[0.950489, 0.310759, 0.855143, 0.511601, 0.083633],atol=1e-6)

        assert_allclose(model.tor_dP_vector[5],[0.309017, -0.951057, 0.881678, -1.401259, -0.509037],atol=1e-6)
    ...
    def test_toroidal_scalar(self,amps_model,mlat,mlt):
        ...
        for i,(n,m) in enumerate(model.keys_T):
            T += P[n,m] * ( model.tor_c[i]*cos(m*mlt/12*pi) +
                            model.tor_s[i]*sin(m*mlt/12*pi) )
...
def test_get_B_ground():
    qdlat,mlt,height,v,By,Bz,tilt,f107 = map(lambda x: np.array(x),[90,0,110,0,0,1,0.5,0.3])
