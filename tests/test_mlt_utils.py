...
def test_geo2mag():
    ...
    assert_allclose(geo2mag(glat=np.array(50*np.pi/180),glon=np.array(50*np.pi/180),epoch=time,deg=False),
                    [(43.660603,),(128.508908,)]
                    )
    assert_allclose(geo2mag(glat=np.array([50,60]),glon=np.array([50,60]),epoch=time,deg=True),
